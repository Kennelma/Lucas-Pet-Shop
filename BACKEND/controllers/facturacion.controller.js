...
async function obtenerSiguienteNumeroFactura(conn) {
  //OBTENER CAI ACTIVO (SIN LOCK PARA VALIDACIONES)
  ...
  if (!cai || cai.length === 0) {
    const error = new Error(
      "NO HAY CAI ACTIVO PARA FACTURAR. CONTACTE AL ADMINISTRADOR."
    );
    error.codigo = "SIN_CAI";
    throw error;
  }
  ...
  //VALIDACIÓN DE CAI VENCIDO (LA DESACTIVACIÓN LA HACE EL JOB AUTOMÁTICO)
  if (fechaLimite < hoy) {
    const error = new Error(
      `CAI VENCIDO (fecha límite: ${caiData.fecha_limite}). NO SE PUEDEN GENERAR FACTURAS. DEBE SOLICITAR UN NUEVO CAI AL SAR.`
    );
    error.codigo = "CAI_VENCIDO";
    throw error;
  }

  //SE VALIDA RANGO (LA DESACTIVACIÓN LA HACE EL JOB AUTOMÁTICO)
  if (caiData.numero_actual > caiData.rango_fin) {
    const error = new Error(
      `FACTURAS AGOTADAS. SE ALCANZÓ EL LÍMITE DE RANGO: (${caiData.rango_fin}). DEBE SOLICITAR UN NUEVO CAI AL SAR.`
    );
    error.codigo = "FACTURAS_AGOTADAS";
    throw error;
  }

  //SE GENERA NUMERO FORMATEADO CON EL PREFIJO DEL CAI
  const numeroFormateado = `${caiData.prefijo}-${String(
    caiData.numero_actual
  ).padStart(8, "0")}`;

  //SE INCREMENTA EL CONTADOR (CON LOCK SOLO AQUI)
  await conn.query(
    `UPDATE tbl_cai
        SET numero_actual = numero_actual + 1
        WHERE id_cai_pk = ? AND activo = TRUE`,
    [caiData.id_cai_pk]
  );
    ...
    for (const item of items) {
      ...
      if (tipo === "PRODUCTOS") {
        ...
        if (producto[0].stock < cantidadSolicitada) {
          errores.push(
            `Stock insuficiente para "${producto[0].nombre_producto}". Solicitado: ${cantidadSolicitada}, Disponible: ${producto[0].stock}`
          );
          continue;
        }
        ...
        if (esMedicamento.length > 0) {
          ...
          if (!lotes || lotes.length === 0) {
            errores.push(
              `No hay lotes disponibles para ${producto[0].nombre_producto}`
            );
            continue;
          }
          ...
          const stockDisponible = lotes.reduce(
            (sum, lote) => sum + lote.stock_lote,
            0
          );
          if (stockDisponible < cantidadSolicitada) {
            errores.push(
              `Stock insuficiente en lotes no vencidos para ${producto[0].nombre_producto}. Disponible: ${stockDisponible}`
            );
            continue;
          }
        }
        ...
        if (!estilistas || estilistas.length === 0) {
          errores.push(
            `El servicio ${servicio[0].nombre_servicio_peluqueria} requiere al menos un estilista`
          );
          continue;
        }
        ...
        if (!estilistas || estilistas.length === 0) {
          errores.push(
            `La promoción ${promocion[0].nombre_promocion} requiere al menos un estilista`
          );
          continue;
        }
...
exports.crearFacturaSinPago = async (req, res) => {
  ...
  try {
    ...
    if (impuesto_porcentaje <= 0 || isNaN(impuesto_porcentaje)) {
      throw new Error(
        "Error al obtener el impuesto del sistema. Contacte al administrador."
      );
    }
      ...
      if (tipo === "PRODUCTOS") {
        ...
        if (esMedicamento.length > 0) {
          ...
          if (cantidadRestante > 0) {
            throw new Error(
              `Stock insuficiente en lotes para "${nombre_item}". Falta: ${cantidadRestante}`
            );
          }
    ...
    const total_final =
      subtotal_exento + subtotal_gravado + impuesto - descuento_valor;
    ...
    const { numero_factura, id_cai } = await obtenerSiguienteNumeroFactura(
      conn
    );
        ...
        if (detalle.lotesADescontar && detalle.lotesADescontar.length > 0) {
          ...
          for (const loteDescontar of detalle.lotesADescontar) {
            ...
            await conn.query(
              ...
              [
                loteDescontar.cantidad,
                loteDescontar.id_lote,
                detalle.id_producto,
              ]
            );
          }
    ...
    res.status(400).json({
      ...
      error: err.message,
      codigo: err.codigo || "ERROR_GENERAL",
    });
  } finally {
    conn.release();
  }
...
exports.crearFacturaConPago = async (req, res) => {
  ...
  try {
      ...
      if (tipo === "PRODUCTOS") {
        ...
        if (esMedicamento.length > 0) {
          ...
          if (stockDisponible < cantidad) {
            throw new Error(
              `Stock insuficiente en lotes no vencidos para ${nombre_item}. Disponible: ${stockDisponible}`
            );
          }
        ...
        if (!servicio || servicio.length === 0) {
          throw new Error(
            `Servicio con ID ${item_id} no encontrado o inactivo`
          );
        }
        ...
        if (!promocion || promocion.length === 0) {
          throw new Error(
            `Promoción con ID ${item_id} no encontrada o inactiva`
          );
        }
    ...
    if (
      isNaN(subtotal_exento) ||
      isNaN(subtotal_gravado) ||
      isNaN(impuesto) ||
      isNaN(total_final)
    ) {
      throw new Error(
        "Error en cálculos finales. Por favor, revise los datos ingresados."
      );
    }
    ...
    if (montoTotalPago > total_final) {
      throw new Error(
        "EL MONTO PAGADO NO PUEDE SER MAYOR AL TOTAL DE LA FACTURA."
      );
    }
...
exports.ImpresionFactura = async (req, res) => {
  ...
  try {
    ...
    const total_pagado = pagos.reduce(
      (sum, pago) => sum + parseFloat(pago.monto_pagado),
      0
    );
    const saldo_pendiente = parseFloat(factura[0].total) - total_pagado;

    // Formatear rango de CAI para el PDF
    let rango_cai = "";
    if (factura[0].prefijo && factura[0].rango_inicio && factura[0].rango_fin) {
      const inicio = `${factura[0].prefijo}-${String(
        factura[0].rango_inicio
      ).padStart(8, "0")}`;
      const fin = `${factura[0].prefijo}-${String(
        factura[0].rango_fin
      ).padStart(8, "0")}`;
      rango_cai = `${inicio} / ${fin}`;
    }
    ...
    res.status(200).json({
      ...
      data: {
        factura: {
          ...
          rango_cai,
          codigo_cai: factura[0].codigo_cai,
          rango_inicio: factura[0].rango_inicio,
          rango_fin: factura[0].rango_fin,
          fecha_limite: factura[0].fecha_limite,
        },
        items: items,
        empresa: empresa[0] || {},
        pagos: pagos,
        resumen: {
          ...
          porcentaje_pagado: (
            (total_pagado / parseFloat(factura[0].total)) *
            100
          ).toFixed(2),
        },
      },
...
exports.validarCAIParaFacturar = async (req, res) => {
  const conn = await mysqlConnection.getConnection();
  ...
  try {
    ...
    const [cai] = await conn.query(`
        SELECT
            ...
            activo
            FROM tbl_cai
        WHERE activo = 1 AND tipo_documento = '01'
        LIMIT 1
    `);

    //SI NO HAY CAI, ACTIVA UNA ALERTA EN FACTURAS

    if (!cai || cai.length === 0) {
      return res.status(200).json({
        puedeFacturar: false,
        mensaje:
          "NO HAY CAI ACTIVO PARA FACTURAR, NO ES POSIBLE REALIZAR FACTURACIÓN.",
        tipoAlerta: "critico",
      });
    }

    const caiData = cai[0];
    const hoy = moment().tz("America/Tegucigalpa").startOf("day");
    const fechaLimite = moment(caiData.fecha_limite)
      .tz("America/Tegucigalpa")
      .startOf("day");

    const facturasRestantes = caiData.rango_fin - caiData.numero_actual + 1;
    const caiVencido = fechaLimite.isBefore(hoy);
    const facturasAgotadas = facturasRestantes <= 0;

    if (caiVencido) {
      return res.status(200).json({
        puedeFacturar: false,
        mensaje: `EL CAI HA VENCIDO, NO ES RECOMENDABLE FACTURAR.`,
        tipoAlerta: "critico",
      });
    }
    ...
    if (facturasAgotadas) {
      return res.status(200).json({
        puedeFacturar: false,
        mensaje:
          "NO QUEDAN FACTURAS DISPONIBLES EN EL RANGO DEL CAI, NO ES RECOMENDABLE FACTURAR.",
        tipoAlerta: "critico",
      });
    }

    //SI TODO ESTA BIEN, ME DEJA FACTURAR
    return res.status(200).json({
      puedeFacturar: true,
      mensaje: "CAI válido. Puede facturar sin advertencias.",
    });
  } catch (error) {
    res.status(500).json({
      puedeFacturar: false,
