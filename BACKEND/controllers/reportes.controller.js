...
//====================REPORTE_DIARIO====================
//OBTIENE EL RESUMEN DE INGRESOS Y GASTOS DEL DÍA ACTUAL
exports.reporteDiario = async (req, res) => {
  ...
  try{
    ...
    //CALCULAR FECHA ACTUAL EN FORMATO YYYY-MM-DD
    let fecha = new Date();
...
//====================REGISTRO_FINANCIERO====================
//OBTIENE INGRESOS, GASTOS Y BALANCE MENSUAL POR AÑO (PARA GRÁFICO DE BARRAS)
    ...
    const [detalles] = await conn.query(`
      SELECT
          ...
          CONCAT(
            ...
            ' | Pago: ', tp.tipo_pago, ' - ', mp.metodo_pago, ' (L.', CAST(pa.monto AS DECIMAL(10,2)), ')'
          ) AS concepto,
          CAST(pa.monto AS DECIMAL(10,2)) AS monto
      FROM tbl_facturas f
      INNER JOIN tbl_pago_aplicacion pa ON f.id_factura_pk = pa.id_factura_fk
      INNER JOIN tbl_pagos p ON pa.id_pago_fk = p.id_pago_pk
  ...
  try {
    ...
    const [detalles] = await conn.query(`
      SELECT
          ...
          f.numero_factura AS documento,
          CONCAT(
            (SELECT GROUP_CONCAT(CONCAT(df.nombre_item, ' (', df.cantidad_item, ') - L.', CAST(df.precio_item AS DECIMAL(10,2))) SEPARATOR ', ')
             FROM tbl_detalles_facturas df
             WHERE df.id_factura_fk = f.id_factura_pk),
