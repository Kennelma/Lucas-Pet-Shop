    ...
    <div className="relative flex justify-center" ref={menuRef}>
      ...
      {isOpen && (
        ...
        >
          ...
          <div
            className={`px-2 py-1.5 text-sm flex items-center gap-2 transition-colors whitespace-nowrap ${rowData.roles && rowData.roles.toLowerCase().includes('administrador') ? 'text-gray-400 cursor-not-allowed bg-gray-100' : 'text-gray-700 hover:bg-red-50 hover:text-red-700 cursor-pointer'}`}
            onClick={(e) => {
              if (rowData.roles && rowData.roles.toLowerCase().includes('administrador')) return;
              e.stopPropagation();
              setIsOpen(false);
              onEliminar(rowData);
            }}
            title={rowData.roles && rowData.roles.toLowerCase().includes('administrador') ? 'NO SE PUEDE ELIMINAR EL USUARIO ADMINISTRADOR' : 'Eliminar'}
          >
...
export default function Usuarios() {
  ...
  const rolesBody = (row) => {
    ...
    else if (roles.toLowerCase().includes('asistente')) colorClasses = 'bg-rose-100 text-rose-800';
  ...
  const handleEliminar = async (usuario) => {
    ...
    if (usuario.roles && usuario.roles.toLowerCase().includes('administrador')) {
      Swal.fire({
        icon: 'warning',
  ...
  return (
    <>
      ...
      <div className="min-h-screen p-3 sm:p-6 bg-gray-50">
        <div className="rounded-xl p-3 sm:p-6 mb-3"
        style={{
      ...
      >
        <div className="flex flex-col justify-center items-center pl-16 pr-1 sm:px-0">
          ...
          <p className="text-center text-black italic mt-1 sm:mt-1 md:mt-2 text-[12px] xs:text-[10px] sm:text-sm md:text-base lg:text-lg leading-tight">
