...
export default function ModalActualizarUsuario({ usuarioData, onClose, onUsuarioActualizado }) {
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      ...
      try {
        ...
        // Si el usuario NO es admin, filtrar el rol admin
        if (Array.isArray(rolesData) && usuarioData && usuarioData.id_rol_fk) {
          const adminRol = rolesData.find(r => r.tipo_rol && r.tipo_rol.toLowerCase() === 'administrador');
          if (!adminRol || usuarioData.id_rol_fk !== adminRol.id_rol_pk) {
  ...
  return (
    <Dialog
      header={<div className="w-full text-center text-sm sm:text-base md:text-lg font-bold" id="modal-title">EDITAR USUARIO</div>}
      visible={true}
      ...
      <div id="modal-content" className="flex flex-col gap-3" role="form" aria-label="Formulario de edición de usuario">
        ...
        <div className="form-field">
          ...
          <Dropdown
            ...
            disabled={(() => {
              const rol = roles.find(r => r.id_rol_pk === form.id_rol_fk);
              return rol && rol.tipo_rol && rol.tipo_rol.toLowerCase() === 'administrador';
            })()}
          />
          {errores.id_rol_fk && (
