...
const ModalPerfilCliente = ({ isOpen, cliente, onClose }) => {
  ...
  const recentPurchases = historialCompras.map((factura) => ({
    ...
    date: new Date(factura.fecha_emision).toLocaleDateString("es-HN", {
      day: "2-digit",
      month: "short",
    }),
  ...
  const selectedOrder = expandedFactura
    ? recentPurchases.find((order) => order.id === expandedFactura)
    : null;

  // Calcular paginación
  const totalPages = Math.ceil(recentPurchases.length / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  const paginatedPurchases = recentPurchases.slice(startIndex, endIndex);
          ...
          {!selectedOrder ? (
            <>
              ...
              <div className="flex gap-1.5 mb-3">
                <div
                  className="bg-gray-50 rounded-lg p-2.5 border border-gray-200 shrink-0"
                  style={{ width: "140px" }}
                >
                ...
                <div className="bg-gray-50 rounded-lg p-2.5 border border-gray-200 flex-1">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-ms font-bold text-gray-800 whitespace-nowrap">
                      L{" "}
                      {totalSpent.toLocaleString("es-HN", {
              ...
              {/* Facturas */}
              <div>
                <p className="text-ls font-semibold text-gray-700 mb-3 text-center">
                      ...
                      >
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <div className="text-sm font-bold text-gray-800">
                            L{" "}
                            {order.total.toLocaleString("es-HN", {
                              minimumFractionDigits: 2,
                            })}
                          </div>
                          <button
                      ...
                      <button
                        ...
                        className={`w-8 h-8 rounded flex items-center justify-center ${
                          currentPage === 1
                            ...
                            : 'bg-green-100 hover:bg-[#DEFFAD] text-green-700'
                        }`}
                      >
              ...
              <div className="mb-3">
                ...
                <div className="bg-gray-50 rounded-lg p-2.5 border border-gray-200 mb-3">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="text-sm font-bold text-gray-800">
                      L{" "}
                      {selectedOrder.total.toLocaleString("es-HN", {
                        minimumFractionDigits: 2,
                      })}
                    </div>
                  </div>
                    ...
                    >
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <div
                          ...
                          style={{ width: "70px" }}
                        >
                          Cant: {detalle.cantidad_item}
                        </div>
                        <div
                          className="text-sm font-bold text-gray-800 text-right"
                          style={{ width: "90px" }}
                        >
                          L{" "}
                          {parseFloat(detalle.total_item).toLocaleString(
                            "es-HN",
                            { minimumFractionDigits: 2 }
                          )}
                        </div>
