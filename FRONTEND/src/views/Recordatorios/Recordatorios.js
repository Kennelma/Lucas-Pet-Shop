...
import { verificarEstado } from '../../AXIOS.SERVICES/whatsapp-axios';
...
//====================VERIFICAR_CONEXION_WHATSAPP====================
const verificarConexionWhatsApp = async () => {
  try {
    const response = await verificarEstado();
    setWhatsappConectado(response.conectado || false);
  } catch (error) {
      ...
      <button
        ...
        className={`${
          whatsappConectado
            ...
            : 'bg-gray-600 hover:bg-gray-700 border-2 border-gray-400'
        } text-white px-4 py-2 rounded-lg font-semibold transition-colors flex items-center gap-2 relative`}
      >
        {whatsappConectado && (
        ...
        {whatsappConectado ? 'WhatsApp Conectado' : 'Conectar WhatsApp'}
      </button>
      <BotonAgregar
        onClick={() => abrirModal()}
        loading={loading}
