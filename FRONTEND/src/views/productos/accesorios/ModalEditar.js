...
const ModalEditar = ({ isOpen, onClose, onSave, editData }) => {
  ...
  useEffect(() => {
    if (isOpen && editData) {
      console.log('editData recibida:', editData); // Debug
      setData({
        nombre: editData.nombre || '', categoria: editData.categoria || 'Collar', cantidad: editData.cantidad || 0,
        precio: editData.precio || 0, imagenUrl: editData.imagenUrl || editData.imagen || ''
      });
    }
    setErrors({});
  }, [isOpen, editData]);
        ...
        <form onSubmit={handleSubmit} style={{display: 'flex'}}>
          <div style={{flex: 1, padding: 20}}>
            <div style={{marginBottom: 16}}>
              ...
              <select name="categoria" value={data.categoria} onChange={handleChange} disabled={!!editData} style={{width: '100%', padding: 8, border: '1px solid #ddd'}}>
