...
const ModalLote = ({ isOpen, onClose, onSave, medicamentoSeleccionado, lotesExistentes = [] }) => {
  ...
  const generarCodigoLote = (nombreMedicamento, lotesDelMedicamento) => {
    // Extraer las primeras 4 letras del nombre (sin espacios)
    const nombreSinEspacios = nombreMedicamento.replace(/\s+/g, '').toUpperCase();
    const letras = nombreSinEspacios.substring(0, 4).padEnd(4, 'X'); // Si tiene menos de 4, rellena con X
    
    // Contar cuántos lotes tiene este medicamento
    const numeroLote = lotesDelMedicamento.length + 1;
    const numeroFormateado = numeroLote.toString().padStart(2, '0');
  ...
  useEffect(() => {
    if (medicamentoSeleccionado && isOpen) {
      // Filtrar los lotes que pertenecen a este medicamento
      const lotesDelMedicamento = lotesExistentes.filter(
        lote => lote.id_producto_fk === medicamentoSeleccionado.id_producto_pk
      );
      
      // Generar código automático
      const codigoAuto = generarCodigoLote(
        medicamentoSeleccionado.nombre_producto,
        lotesDelMedicamento
      );
