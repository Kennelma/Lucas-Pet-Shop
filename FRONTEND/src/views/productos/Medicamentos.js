...
const InventarioMedicamentos = () => {
  ...
  useEffect(() => {
    const inventarioGuardado = JSON.parse(localStorage.getItem('inventario-medicamentos')) || [];
  ...
  const migrarInventario = (inventarioData) => {
    ...
    const inventarioMigrado = inventarioData.map(producto => {
      ...
      // Asegurar que todos los productos sean de categoría Medicamento
      producto.categoria = 'Medicamento';
      return producto;
    });
    
    if (necesitaMigracion) {
      localStorage.setItem('inventario-medicamentos', JSON.stringify(inventarioMigrado));
    }
    setInventario(inventarioMigrado);
  };

  const generarCodigo = (categoria, inventarioData = inventario) => {
    const prefijo = prefijosCodigo[categoria] || 'MD';
  ...
  const abrirModal = () => {
    ...
    setEditIndex(-1);
    setFormData({
      codigo: generarCodigo('Medicamento'),
      nombre: '',
      categoria: 'Medicamento',
      cantidad: 0,
      precio: 0
  ...
  const registrarInventario = () => {
    ...
    if (!nombre.trim()) {
      mostrarMensaje('Por favor ingresa el nombre del medicamento.', 'error');
      return;
    }
    
    if (!codigo.trim()) {
      mostrarMensaje('Error generando el código del medicamento.', 'error');
      return;
    }
    ...
    if (editIndex === -1 && inventario.some(p => p.codigo === codigo)) {
      mostrarMensaje('Ya existe un medicamento con este código.', 'error');
      return;
    }
    ...
    localStorage.setItem('inventario-medicamentos', JSON.stringify(nuevoInventario));
    cerrarModal();
  };
  ...
  const borrarInventario = (index) => {
    ...
    if (window.confirm(`¿Deseas eliminar "${producto.nombre}" (${producto.codigo}) del inventario?`)) {
      ...
      setInventario(nuevoInventario);
      localStorage.setItem('inventario-medicamentos', JSON.stringify(nuevoInventario));
      mostrarMensaje('Medicamento eliminado correctamente.');
    }
  };
  ...
  return (
    <div className={`min-h-screen p-5 font-sans transition-all duration-300 ${
      ...
      <div className="flex justify-between items-center mb-5">
        ...
        <button
          onClick={abrirModal}
          className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white border-none rounded cursor-pointer transition-all duration-200"
        >
          + NUEVO MEDICAMENTO
        </button>
      </div>
      ...
      <div className="mb-4">
        <input
          ...
          className={`px-3 py-2 w-64 mr-3 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-200 ${
            isDark 
      ...
      {productosFiltrados.length === 0 ? (
        <div className={`text-center mt-12 ${
          ...
          <p className={isDark ? 'text-gray-400' : 'text-gray-500'}>
            Agrega tu primer medicamento usando el botón "Nuevo Medicamento"
          </p>
        </div>
      ) : (
          ...
          <div className={`p-5 rounded-lg w-full max-w-md mx-4 shadow-2xl ${
            ...
            <div className="flex justify-between items-center mb-5">
              <h2 className={`text-xl font-bold m-0 ${
                ...
                {editIndex >= 0 ? 'EDITAR MEDICAMENTO' : 'NUEVO MEDICAMENTO'}
              </h2>
              <span
                onClick={cerrarModal}
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full px-3 py-2 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors duration-200 ${
                    isDark 
              ...
              <button
                ...
                className="w-full px-4 py-3 bg-blue-600 hover:bg-blue-700 text-white border-none rounded cursor-pointer transition-all duration-200 font-medium"
              >
                {editIndex >= 0 ? 'ACTUALIZAR MEDICAMENTO' : 'AGREGAR MEDICAMENTO'}
              </button>
            </div>
          </div>
        </div>
