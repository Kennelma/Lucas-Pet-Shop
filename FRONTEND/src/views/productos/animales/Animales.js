...
const Animales = () => {
  ...
  // 🔹 Carga los productos de tipo "animales"
      ...
      setAnimales(
        Array.isArray(productos)
          ? productos.map((item, index) => ({
              id: item.sku || index, // usa SKU como identificador si no hay id_producto_pk
  ...
  return (
    <div className="min-h-screen p-5 bg-white">
      ...
      <div className="mb-6 relative w-80">
        ...
        />
        {busqueda && (
          <button onClick={() => setBusqueda('')} className="absolute right-3 top-2">
            ×
          </button>
        )}
      </div>
      ...
      {animalesFiltrados.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {animalesFiltrados.map((a, index) => (
            <div key={a.id} className="bg-gray-100 rounded-lg p-4 relative shadow-sm">
              ...
              <div className="text-center mb-8">
                ...
                <div
                  className={`text-sm ${
                    a.cantidad < a.stock_minimo ? 'text-red-600' : 'text-gray-600'
                  }`}
                >
                  Stock: {a.cantidad} (Mín: {a.stock_minimo})
                </div>
                <div
