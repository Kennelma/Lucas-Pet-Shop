          ...
          <form onSubmit={handleSubmit}>
            <div className="form-group">
              ...
              <input
                ...
                className={`form-input ${errores.nombre_servicio_peluqueria ? 'input-error' : ''}`}
                placeholder="Ej: Baño completo con corte"
              />
              {errores.nombre_servicio_peluqueria && (
                <p className="form-error">{errores.nombre_servicio_peluqueria}</p>
              )}
            </div>
            ...
            <div className="form-group">
              ...
              <textarea
                ...
                onChange={handleChange}
                className={`form-input form-textarea ${errores.descripcion_servicio ? 'input-error' : ''}`}
                placeholder="Descripción detallada del servicio..."
              />
              {errores.descripcion_servicio && (
                <p className="form-error">{errores.descripcion_servicio}</p>
              )}
            </div>
            
            <div className="form-group">
              <label htmlFor="precio_servicio" className="form-label">
                Precio del Servicio (L) <span style={{ color: '#ef4444' }}>*</span>
              </label>
              <input
                type="number"
                step="0.01"
                ...
                onChange={handleChange}
                className={`form-input ${errores.precio_servicio ? 'input-error' : ''}`}
                placeholder="0.00"
              />
              {errores.precio_servicio && (
                <p className="form-error">{errores.precio_servicio}</p>
              )}
            </div>

            <div className="form-group">
              <label htmlFor="duracion_estimada" className="form-label">
                Duración Estimada (minutos) <span style={{ color: '#ef4444' }}>*</span>
              </label>
              <input
                type="number"
                ...
                onChange={handleChange}
                className={`form-input ${errores.duracion_estimada ? 'input-error' : ''}`}
                placeholder="Ej: 60"
              />
              <p className="form-hint">Tiempo aproximado en minutos (mín: 5, máx: 480)</p>
              {errores.duracion_estimada && (
                <p className="form-error">{errores.duracion_estimada}</p>
              )}
            </div>
