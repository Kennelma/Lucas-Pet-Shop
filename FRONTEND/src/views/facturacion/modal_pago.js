...
import { procesarPago, obtenerMetodosPago } from '../../AXIOS.SERVICES/payments-axios';
...
function ModalPago({ show, numero_factura, total, onClose, onSuccess }) {
  ...
  useEffect(() => {
    const cargarMetodosPago = async () => {
      ...
      if (response.success) {
        setMetodosPagoDB(response.data);
      }
    };
    cargarMetodosPago();
  }, []);
  ...
  const handleProcessPayment = async () => {
    ...
    setLoading(true);
    try {
      // Construir array de métodos con sus montos
      const metodos = selectedPaymentMethods.map(methodId => ({
        metodo: methodId,
      ...
      const datosPago = {
        numero_factura,
        tipo_pago: paymentType === 'total' ? 'TOTAL' : 'PARCIAL',
        metodos
      };
